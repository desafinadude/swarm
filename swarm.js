...
class Swarm {
    ...
    constructor(canvas, options){
        ...
        this.particles = [];
        for (var i = 0; i < (options.particles || Math.max(Math.floor(this.scale.x * this.scale.y / 4000), 90)); i++) {
...
class PointRenderer extends ParticlePlugin{
    constructor(context, status = undefined, name = undefined, fillStyle = '#999'){
...
class LineRenderer extends ParticlePlugin{
    constructor(context, status = undefined, name = undefined, lineRed = 200, lineGreen = 200, lineBlue = 200, lineWidth = 2){
        super(context, status, name);
        this.setLineStyle(lineRed, lineGreen, lineBlue);
...
var swarm = new Swarm( document.getElementById('swarm'), {} );

swarm
    .addParticlePlugin(new CenterGravity(swarm,true))
    .addParticlePlugin(new ParticleSpeedLimit(swarm,false))
    .addParticlePlugin(new LineToMouseRenderer(swarm,true))
